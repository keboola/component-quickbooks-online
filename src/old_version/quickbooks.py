...
class quickbooks():
    ...
    def __init__(self, company_id):
        ...
        """if endpoint=="Custom Query":
            logging.info("Entered Query: {0}".format(query))
            self.custom_request()
            self.json_output()"""

        """if report_api_bool:
            logging.info("Processing Report: {0}".format(endpoint))
            self.report_request(endpoint)
        else:
    ...
    def refresh_access_token(self):
        ...
        app_key_secret = "{0}:{1}".format(app_key, app_secret)  # noqa
    ...
    def get_count(self):
        ...
        logging.info(data)
        """if "fault" in data or "Fault" in data:
            if not self.refresh_token_bool:
                self.refresh_access_token()
                total_counts = self.get_count()
            else:
    ...
    def _request(self, url):
        ...
        while not request_success:
            ...
            # Outputting IntuitID
            # if 'intuit_tid' in data.headers:
            #    logging.info('Intuit_tid: {}'.format(data.headers['intuit_tid']))
            ...
            if "fault" in results or "Fault" in results:
                ...
                # logging.info("_request: No fault found")
                request_success = True
        # results = json.loads(data.text)
    ...
    def json_output(self):
        ...
        print(data)
        for i in data:
            # data[self.endpoint] = self.data
            # with open("/data/out/tables/{0}_{1}.json".format("custom", i), "w") as f:
            #    json.dump(data[i], f)
